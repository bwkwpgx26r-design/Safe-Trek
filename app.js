/* ============================================================
   SafeTrek – app.js v10 (mit Back-Navigation Stack)
   ...
   - ✅ Neu: Zurück-System (navStack + goBack)
   ============================================================ */

console.log("SafeTrek app.js loaded v10 + back");
...
const state = {
  screen: "splash",
  navStack: [], // ✅ NEW: back navigation stack
...
function go(id, options = { replace:false }) {
  if (!options.replace && state.screen && state.screen !== id) {
...
window.addEventListener("load", () => {
  // ✅ replace:true so you don't "back" into splash
  setTimeout(() => go("auth", { replace:true }), 1200);
});
...
/* ------------------ Public API for HTML ------------------ */
window.SafeTrek = {
  goBack, // ✅ NEW: can be used by any back button in HTML
...
async function buildAndShowRoute(pick, cardEl){
  ...
  if(cardEl){
    ...
    if(wxEl){
      ...
      wxEl.textContent = w?.tempC != null ? `Wetter ${Math.round(w.tempC)}° · ${w.desc}` : `Wetter —`;
    }
  }
}
...
function updateSummaryUI(){
  ...
  const wx = r.weather;
  if(wxEl) wxEl.textContent = wx?.tempC != null ? `Wetter ${Math.round(wx.tempC)}° · ${wx.desc}` : "Wetter —";
  if(elEl) elEl.textContent = r.elevUpM != null ? `Höhenmeter ↑ ${Math.round(r.elevUpM)} m` : "Höhenmeter —";
  if(exEl) exEl.textContent = `Exits ${exits}`;
}
...
function updateNavUI(){
  // MVP: nothing to compute here; user has the "can't continue" button
