console.log("SafeTrek app.js loaded v8");
...
const state = {
  ...
  radiusKm: 30,              // ✅ user selectable
  userPinnedCenter: null,    // ✅ if using current location
...
function selectWizard(step, value){
  ...
  if(step < 3){
    ...
    // ✅ After check-in: ask for region + radius (again)
    go("region");
  }
}
...
window.SafeTrek = {
  setRegion: (key) => {
    ...
    state.userPinnedCenter = null; // choosing region overrides pinned
    renderRegionSummary();
  },
  setRadiusKm: (km) => {
    state.radiusKm = km;
  ...
  useMyLocation: async () => {
    const pos = await getGeoOnce(6000);
    if(pos){
      state.userPinnedCenter = { lat: pos.lat, lon: pos.lon };
      // still pick nearest region label for UX
...
async function loadRouteCards(forceReload){
  ...
  // ✅ Center and radius from selection
  const center = state.userPinnedCenter
...
function makeRouteCard(pick){
  ...
  c.querySelector('[data-act="preview"]').onclick = async () => buildAndShowRoute(pick, c);
  c.querySelector('[data-act="select"]').onclick  = async () => buildAndShowRoute(pick, c);
...
function weatherCodeToText(code){
  // Simple, calm descriptions (enough for MVP)
  const c = Number(code);
...
/* ------------------ Build full route + update UI (incl weather/elev) ------------------ */
async function buildAndShowRoute(pick, cardEl){
  ...
  // Map info: loading
  setMapInfo("Lade Route…", "Wetter …", "Höhenmeter …");
  ...
  const wx = route.weather;
  const wxText = wx?.tempC != null
    ? `${Math.round(wx.tempC)}° · ${wx.desc}${wx.windKmh!=null ? ` · Wind ${Math.round(wx.windKmh)} km/h` : ""}`
    : "Wetterdaten nicht verfügbar";

  const elevText = route.elevUpM != null
    ? `↑ ${Math.round(route.elevUpM)} m${route.elevDownM!=null ? ` · ↓ ${Math.round(route.elevDownM)} m` : ""}`
    : "Höhendaten nicht verfügbar";

  setMapInfo(
    `${route.lengthKm ? round1(route.lengthKm) + " km" : "—"} · ${(route.exits||[]).length} Ausstiege in der Nähe`,
    `Wetter ${wx?.tempC!=null ? Math.round(wx.tempC)+"° · "+wx.desc : "—"}`,
    `Höhenmeter ${route.elevUpM!=null ? "↑ "+Math.round(route.elevUpM)+" m" : "—"}`
  );

  // Also set detailed line text
  const mapInfoText = document.getElementById("mapInfoText");
  if(mapInfoText){
    mapInfoText.textContent = `${wxText} · ${elevText}`;
  }
}

function setMapInfo(text, pillWx, pillElev){
  const mapInfoText = document.getElementById("mapInfoText");
  const pWx = document.getElementById("pillWx");
  const pEl = document.getElementById("pillElev");
  if(mapInfoText) mapInfoText.textContent = text || "";
  if(pWx) pWx.textContent = pillWx || "Wetter —";
  if(pEl) pEl.textContent = pillElev || "Höhenmeter —";
}
