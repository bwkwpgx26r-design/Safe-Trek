/* ============================================================
   SafeTrek – app.js v11
   ✅ Visuell NICHTS ändern (nur Logik/Flow)
   ✅ Back/Zurück von jeder Option (saubere History)
   ✅ Wizard -> Region (Region+Radius Highlight)
   ✅ Routes -> Route wählen -> Summary -> Pack -> Navigation
   ✅ Wetter (Open-Meteo) + Höhenmeter (OpenTopoData)
   ✅ Map: Leaflet, Plan A/B/C (optional via applyPlan), SOS Overlay
   ============================================================ */

console.log("SafeTrek app.js loaded v11");
...
/* ------------------ State ------------------ */
const state = {
  // screen + navigation history (new)
  screen: "splash",
  navStack: [],           // stack of visited screens
  lastBackAt: 0,          // prevent accidental double-back
...
/* ============================================================
   1) Navigation (Back / History)
   ============================================================ */

/**
 * go(screenId, opts)
 * - push to history by default
 * - opts.replace: replace current (no push)
 * - opts.noPush: do not push history
 */
function go(id, opts = {}){
  const { replace=false, noPush=false } = opts;
  ...
  if(!noPush){
    if(replace){
      ...
      // avoid pushing same screen twice
      if(prev && prev !== id){
  ...
  if(id === "routes"){
    // nothing special; cards render by loadRoutes
  }
...
/**
 * back() - global back that works from any screen.
 * If stack empty -> go home.
 */
function back(){
  const now = Date.now();
  if(now - state.lastBackAt < 250) return;
  state.lastBackAt = now;

  // close overlays first
  if(document.getElementById("sosOverlay")){
    removeSOSOverlay();
    return;
  }
...
// allow browser back on web (optional but helpful)
window.addEventListener("popstate", () => back());

/* Expose for HTML buttons if needed */
window.back = back;

/* ------------------ Basic actions ------------------ */
function enterAsGuest(){ go("home", { replace:true }); }
function noop(){}

/* Splash -> auth */
window.addEventListener("load", () => {
  // Do NOT push splash into history
  setTimeout(() => go("auth", { replace:true, noPush:true }), 1200);
});
...
function resetWizard(){
  state.wizardStep = 0;
  state.wizard = [null,null,null,null];
  document.querySelectorAll(".wizard-step").forEach(s => s.classList.remove("active"));
  const first = document.querySelector('.wizard-step[data-step="0"]');
...
/* ============================================================
   5) Route cards UI CSS (injected; does not affect global layout)
   ============================================================ */
injectRouteCardCSS();
...
function makeSkeletonCard(){
  ...
  c.innerHTML = `
    ...
    <div class="stActions">
      <button class="stBtn" disabled>Route ansehen</button>
      <button class="stBtn primary" disabled>Route wählen</button>
    </div>
  `;
  return c;
}
...
function computeSafetyBadge(){
  ...
  if(conservative){
    return { cls:"warn", badge:"konservativ", label:"Safety-first", text:"Heute konservativ planen – sichere Ausstiege priorisiert" };
  }
  return { cls:"good", badge:"gut machbar", label:"Balanced", text:"Gute Basis – Alternativen & Exit-Optionen werden berücksichtigt" };
}
...
function updateSummaryUI(){
  ...
  if(text) text.textContent = `${km} · ${elev} · ${exits} Ausstiege in der Nähe.`;
...
function updateNavUI(){
  // MVP nav screen: just state; optional map follow not implemented yet
...
function showSOSOverlay(){
  ...
  sheet.appendChild(btn("Zurück", () => { removeSOSOverlay(); }));
