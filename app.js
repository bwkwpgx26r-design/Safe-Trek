/* ============================================================
   SAFETREK – app.js (A+B UI: Komoot + Apple)
   - Premium Route Cards (Title + Region + Stats + Safety text)
   - Mini-map preview (canvas) for trust
   - Buttons with labels (Route ansehen / Route wählen)
   - Keeps map/plans/SOS functions working with your Step-1 UI
   ============================================================ */
...
const state = {
  screen: "auth",
  wizardStep: 0,
  wizard: [null,null,null,null], // mood, stamina, breaks, safety
...
/* =========================
   PREMIUM UI (Injected CSS)
   ========================= */
injectPremiumUI();

function injectPremiumUI(){
  if(document.getElementById("st_premium_css")) return;
  const css = document.createElement("style");
  css.id = "st_premium_css";
  css.textContent = `
    :root{
      --st-bg: #0b0b0b;
      --st-card: rgba(255,255,255,0.06);
      --st-card2: rgba(255,255,255,0.04);
      --st-line: rgba(255,255,255,0.10);
      --st-text: rgba(255,255,255,0.92);
      --st-sub: rgba(255,255,255,0.68);
      --st-sub2: rgba(255,255,255,0.52);
      --st-accent: rgba(78,161,255,0.55);
      --st-good: rgba(50,213,131,0.70);
      --st-warn: rgba(255,204,0,0.70);
    }
    ...
    .stCard{
      ...
      background: linear-gradient(180deg, var(--st-card), var(--st-card2));
      border-radius:20px;
      padding:14px;
      display:flex;
    ...
    .stBadge.good{ border-color: var(--st-good); }
    .stBadge.warn{ border-color: var(--st-warn); }
...
`;
  ...
  try{
    ...
    document.getElementById("stReload").onclick = () => loadRouteCards();
    return;
  }
  ...
  cards.innerHTML = "";
  state.routePicks.forEach((pick) => cards.appendChild(makeRouteCard(pick)));

  // Lazy: draw preview quickly, then improve with real preview geometry
  const cardEls = [...cards.children];
  for(let i=0;i<state.routePicks.length;i++){
    const pick = state.routePicks[i];
    const card = cardEls[i];
    const canvas = card?.querySelector("canvas");
    drawMiniPreview(canvas, null); // immediate
...
function makeSkeletonCard(){
  ...
  c.innerHTML = `
    <div class="stHeader">
      <div class="stTitle">Lade Routen…</div>
      <div class="stMeta">
    ...
    <div class="stSafety">
      <span>Safety-Check wird vorbereitet</span>
...
function makeRouteCard(pick){
  ...
  const safety = computeSafetyBadge(); // based on wizard values (calm)
  c.innerHTML = `
    <div class="stHeader">
    ...
    <div class="stSafety">
      ...
      <div class="stBadge ${safety.cls}">${safety.badge}</div>
    </div>
...
/* Safety badge based on wizard (lightweight, readable) */
function computeSafetyBadge(){
  // wizard: mood (0), stamina (1), breaks (2), safety (3) each 1..4
  const mood = state.wizard[0] ?? 2;
  const stamina = state.wizard[1] ?? 2;
  const breaks = state.wizard[2] ?? 2;
  const safety = state.wizard[3] ?? 2;

  // score: higher = stronger capacity; safety request reduces aggressiveness
  const capacity = (mood + stamina + breaks) / 3;
  const caution = safety; // higher means wants safer

  // Determine badge
  if(caution >= 3 || capacity <= 2){
    return {
      cls: "warn",
      badge: "konservativ",
      label: "Safety-first",
      text: "Heute eher konservativ planen – sichere Ausstiege priorisiert"
    };
  }
  return {
    cls: "good",
    badge: "gut machbar",
    label: "Balanced",
    text: "Gute Basis für eine sichere Tour – Alternativen verfügbar"
  };
}

/* =========================
   PREVIEW GEOMETRY (for mini map)
   ========================= */
...
/* =========================
   BUILD FULL ROUTE + Update card stats
   ========================= */
async function buildAndShowRoute(pick, cardEl){
  ...
  // Update stats on card (if still in DOM)
  if(cardEl){
    const dist = cardEl.querySelector('[data-chip="dist"]');
    const up = cardEl.querySelector('[data-chip="up"]');
    const ex = cardEl.querySelector('[data-chip="ex"]');

    if(dist) dist.textContent = `📏 km ${route.lengthKm ? round1(route.lengthKm) : "—"}`;
    if(up) up.textContent = `↑ m ${route.elevUpM != null ? Math.round(route.elevUpM) : "—"}`;
    if(ex) ex.textContent = `🚪 exits ${(route.exits||[]).length}`;
  }
}
...
function clearMapLayers(){
  ...
  state.markerHighlight?.remove(); state.markerHighlight=null;
}
...
function applyPlan(p){
  ...
  const color = (p==="A") ? "#32d583" : (p==="B") ? "#32d583" : "#ffcc00";
...
/* SOS Overlay (kept minimal; already professional) */
